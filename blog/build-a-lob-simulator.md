...
On a 200k-order stream, the pure-Python engine processes roughly **22k events/sec** while the C++ engine reaches **90k events/sec**. The bigger win will come from a batch-submission API that keeps more work on the C++ side; that is on the roadmap.
...
- Publishing the post on Medium / Dev.to.
- LinkedIn and X launch.
- Pinning the repository on my GitHub profile.
- Publishing the package to PyPI (the wheel and CI are ready).
